...
def get_network(args, net, use_gpu=True, gpu_device = 0, distribution = True):
    ...
    if net == 'sam2':
        if args.version == 'Medical_sam2':
            from medical_sam2_train.build_sam import build_sam2_video_predictor
...
def create_logger(log_dir, phase='train'):
    ...
    logger.file_handler = logging.FileHandler(final_log_file, mode='w')
    console = logging.StreamHandler()
    logging.getLogger('').addHandler(console)
    logger.addHandler(logger.file_handler)
    logger.info('Start logging at {}'.format(final_log_file))
...
def generate_bboxes(mask, variation=0, seed=None):
    """
    mask: 2D numpy array of integer labels.
        Assumes the object of interest uses the max label (max_label).
    variation: scale factor for random bbox perturbation (default=0: none)
    seed: random seed (default=None)
    
    Returns:
          bboxes: (N,4) numpy array of [y0, x0, y1, x1] for each connected component.
              N is the number of disconnected components.
              Returns an empty (0,4) array when nothing is found.
    """
    if seed is not None:
        np.random.seed(seed)
    if mask.ndim != 2:
        raise ValueError(f"Mask shape must be 2D, but got {mask.shape}")
    
    # Label for the object of interest (largest value)
    max_label = int(mask.max())
    if max_label == 0:
        # No object present
        return np.zeros((0, 4), dtype=float)
    
    # binary mask for our object
    obj = (mask == max_label).astype(np.uint8)
    # disconnected component labeling
    comp_map, num_comps = label(obj)
    if num_comps == 0:
        return np.zeros((0, 4), dtype=float)
    
    bboxes = []
    for comp_id in range(1, num_comps + 1):
        inds = np.argwhere(comp_map == comp_id)
        x0, x1 = inds[:, 0].min(), inds[:, 0].max()
        y0, y1 = inds[:, 1].min(), inds[:, 1].max()
        w, h = x1 - x0, y1 - y0
        mid_x, mid_y = (x0 + x1) / 2, (y0 + y1) / 2
        
        # Apply bbox size variation
        if variation > 0:
            delta_w, delta_h = np.random.randn(2) * variation
            w *= (1 + delta_w)
            h *= (1 + delta_h)
            x0 = mid_x - w / 2
            x1 = mid_x + w / 2
    ...
    return np.array(bboxes, dtype=float)
...
def generate_bbox(mask, variation=0, seed=None):
    ...
    # Apply random variation when requested (two random factors)
    if variation > 0:

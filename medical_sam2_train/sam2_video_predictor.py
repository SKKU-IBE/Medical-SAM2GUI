...
from medical_sam2_train.modeling.sam2_base import NO_OBJ_SCORE, SAM2Base
from medical_sam2_train.utils.misc import concat_points, fill_holes_in_mask_scores, load_video_frames, load_video_frames_from_data
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def val_init_state(
        ...
        inference_state = {}
        inference_state["images"] = images.to(dtype=torch.float32)
        inference_state["num_frames"] = len(images)
        # whether to offload the video frames to CPU memory
        # turning on this option saves the GPU memory with only a very small overhead
        inference_state["offload_video_to_cpu"] = offload_video_to_cpu
